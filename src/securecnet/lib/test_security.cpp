...
static Endpoint make_ipv4_loopback_endpoint(U16 port) {
    Endpoint ep{};
...
struct ManualClient {
    ...
    Result open(const Endpoint& server_ep) {
        server = normalize_test_target(server_ep);
...
int test_security() {
    ...
    {
        Server srv;
        auto rc = srv.listen(make_ipv4_loopback_endpoint(0));
...

    {
        ...
        Endpoint server_target = ManualClient::normalize_test_target(server_local);

        UdpSocket sock{};
        fails += expect(sock.open(reinterpret_cast<const sockaddr*>(&server_target.addr)->sa_family).ok(), "malformed-handshake: socket open failed");
        std::array<U8, NetConfig::MaxPacketBytes> packet{};
        PacketHeader header{};
        header.kind = static_cast<U8>(PacketKind::Handshake);
        header.seq = 1;
        ...
        fails += expect(sock.send_to(server_target, packet.data(), packet_len).ok(), "malformed-handshake: send failed");
...

    {
        ...
        fails += expect(cli.sock.send_to(cli.server, replay_packet.data(), replay_len).ok(), "replay/auth: first replay send failed");
        fails += expect(srv.tick().ok(), "replay/auth: first replay tick failed");
        fails += expect(cli.sock.send_to(cli.server, replay_packet.data(), replay_len).ok(), "replay/auth: duplicate replay send failed");
        ...
        fails += expect(cli.sock.send_to(cli.server, bad_mac_packet.data(), bad_mac_len).ok(), "replay/auth: bad-MAC send failed");
...

    {
        ...
        fails += expect(cli.sock.send_to(cli.server, wrong_packet.data(), wrong_len).ok(), "wrong-key: send failed");
